...
TARGETDIST="$HOME/.resolve-up.bash"
TARGETSIM='/usr/local/bin/resolve-up'
HL='\033[1;33m' # Yellow
ER='\033[0;31m' # Red
NC='\033[0m' # No Color
...
# TASKS
cleanTargetDist() {
    # If ~/.resolve-up.bash already exists we should remove it
    dotEffect
    rm -r "$TARGETDIST"
    mkdir "$TARGETDIST"
    dotEffect
...
copyDist() {
    dotEffect
    cp -R "$CURERNTDIR/." "$HOME/.resolve-up.bash"
    dotEffect
}

linkCommand() {
    dotEffect
    cp "$TARGETDIST/resolve-up.bash" $TARGETSIM
    dotEffect
}
...
patchBashrc() {
    ...
    dotEffect
    if ! hide=$(grep -R "source ~/.resolve-up.bash/resolve-up-init.sh" "$HOME/.bashrc")
    then
        printf '\n\n'$"# Initialize resolve-up.bash" >> "$HOME/.bashrc"
        printf "\nsource ~/.resolve-up.bash/resolve-up-init.sh" >> "$HOME/.bashrc"
        dotEffect
    fi
    dotEffect
}
...
# Ensure that distribution folder located in ~/.resolve-up.bash
...
if [ -L "$TARGETSIM" ] || [ -f "$TARGETSIM" ] || [ -d "$TARGETSIM" ]; then
    if ! [ $FORCE ]; then
        clear
        printf "Seems like command ${HL}resolve-up${NC} already exists in ${HL}$TARGETSIM${NC}. This program requires that name.\n"
...
clear
## Intro:
printf "Install ${HL}resolve-up.bash${NC} to your machine:\n"
...
sh "$HOME/.bashrc"
printf "\n"
printf "Command ${HL}resolve-up${NC} now aviable in your CLI\n"
printf "\n"
